...
public class CompanyResource {
	...
	@PostMapping(value = "/empresas/{cnpj}")
	...
	public ResponseEntity<String> updateCompany(@PathVariable String cnpj, @RequestBody CompanyDTO companyDto) {
		try {
	...
	@DeleteMapping("/empresas/{cnpj}/pacotes/{packageNumber}")
	public ResponseEntity<String> deleteCompanyByIdAndCnpj(@PathVariable String cnpj,
			@PathVariable Long packageNumber) {
		try {
			cnpj = URLDecoder.decode(cnpj, StandardCharsets.UTF_8.toString());
			companyService.deletePackageById(packageNumber);
